...
class DensePoseResultsVisualizer:
    def visualize(
        ...
        for i, result in enumerate(densepose_result):
            ...
            ret = self.visualize_iuv_arr(context, iuv_array.cpu().numpy(), boxes_xywh[i])
        image_bgr = self.context_to_image_bgr(context) if ret is None else ret
        return image_bgr
...
class DensePoseMaskedColormapResultsVisualizer(DensePoseResultsVisualizer):
    ...
    def visualize_iuv_arr(self, context, iuv_arr: np.ndarray, bbox_xywh) -> None:
        ...
        label_dict = {
            ...
            'right_lower_leg': [13],
            'left_lower_leg': [14],
            'left_upper_arm': [15, 17],
            'right_upper_arm': [16, 18],
            'left_lower_arm': [19, 21],
            'right_lower_arm': [20, 22],
            'head': [23, 24]
        }

        label_list = list(np.unique(matrix))
        label_list = [l for l in label_list if l != 0]
        label_list.sort()

        label_dict_list = []
        for value_list in label_dict.values():
            label_dict_list.extend(value_list)
        label_dict_list.sort()

        # assert label_list == label_dict_list, f'{label_list} != {label_dict_list}'

        image_dict = {}
        for label_name, label_index_list in label_dict.items():
            mask = np.zeros(matrix.shape, dtype=np.uint8)
            for l in label_index_list:
                mask[(matrix == l) & (segm > 0)] = 1

            ret = self.mask_visualizer.visualize(copy.deepcopy(image_bgr), mask, copy.deepcopy(matrix), bbox_xywh)
            if np.max(ret) != 0:
                image_dict[label_name] = ret
