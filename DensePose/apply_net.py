...
class DumpAction(InferenceAction):
    ...
    def postexecute(cls: type, context: Dict[str, Any]):
        ...
        with open('./data.json','w') as f:
            json.dump([context["pred_densepose"][0].labels.cpu().tolist(), context["pred_densepose"][0].uv.cpu().tolist(), context["pred_boxes_XYXY"][0].cpu().tolist()], f)
...
class ShowAction(InferenceAction):
    ...
    def execute_on_outputs(
        ...
        if len(out_dir) > 0 and not os.path.exists(out_dir):
            os.makedirs(out_dir)
        if type(image_vis) == dict:
            for k, v in image_vis.items():
                dirname = os.path.dirname(out_fname)
                basename = os.path.basename(out_fname).split('.')
                basename[-2] = k
                basename = '.'.join(basename)
                cv2.imwrite(os.path.join(dirname, basename), v)
        else:   
